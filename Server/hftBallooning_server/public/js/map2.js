...
function Map(div_id, startLat, startLon)
{
	...
	function animateTo(oldPosition, newPosition, duration_ms) 
	{
		...
		var animateStep = function (startTime) 
		{
			...
			var durationRatio = ellapsedTime / duration_ms; // 0 - 1
			var easingDurationRatio = durationRatio;

			if (durationRatio < 1) 
			{
				deltaPosition = new OpenLayers.LonLat
				(
					oldPosition.lon + (newPosition_lng - oldPosition.lon) * easingDurationRatio,
					oldPosition.lat + (newPosition_lat - oldPosition.lat) * easingDurationRatio
					
				)/*.transform(epsg4326, map.getProjectionObject())*/;

				balloon.move(deltaPosition);
				featureCircle.move(deltaPosition);
...
// 95% confidence circle radius from DHOP and CET
